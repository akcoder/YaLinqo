...
trait EnumerablePagination {
     ...
     *
     * @throws Exception|UnexpectedValueException if sequence does not contain value with specified key
     *
     * @return mixed the value at the key in the source sequence
     */
    public function elementAt(mixed $key): mixed {
     ...
     *
     * @param mixed      $key     the key of the value to retrieve
     * @param mixed|null $default Value to return if sequence does not contain value with specified key. Default: null.
     *
     * @throws Exception
     *
     ...
     */
    public function elementAtOrDefault(mixed $key, mixed $default = null): mixed {
        /** @var $it Iterator|ArrayAccess */
        $it = $this->getIterator();
     ...
     */
    public function firstOrFallback(callable $fallback, ?callable $predicate = null): mixed {
        $predicate = $predicate ?? Functions::$true;
     ...
     */
    public function lastOrFallback(callable $fallback, ?callable $predicate = null): mixed {
        $predicate = $predicate ?? Functions::$true;
     ...
     *
     * @param callable|null $fallback  {() ==> value} A fallback function to return the default element
     * @param callable|null $predicate {(v, k) ==> result} A function to test each element for a condition. Default: true.
     *
     * @throws UnexpectedValueException if source contains more than one matching element
     *
     ...
     */
    public function singleOrFallback(?callable $fallback, callable $predicate = null): mixed {
        $predicate = $predicate ?? Functions::$true;
     ...
     *
     * @return false|int|string the key of the first occurrence of value, if found; otherwise, false
     */
    public function indexOf(mixed $value): bool|int|string {
     ...
     */
    public function findIndex(callable $predicate): string|int|null {
        foreach ($this as $k => $v) {
            if ($predicate($v, $k)) {
                return $k;
            }
     ...
     */
    public function findLastIndex(callable $predicate): string|int|null {
        $key = null;
        foreach ($this as $k => $v) {
            if ($predicate($v, $k)) {
                $key = $k;
     ...
     */
    public function skipWhile(callable $predicate): Enumerable {
     ...
     */
    public function takeWhile(callable $predicate): Enumerable {
        return new self(function() use ($predicate) {
            foreach ($this as $k => $v) {
                if (!$predicate($v, $k)) {
                    break;
